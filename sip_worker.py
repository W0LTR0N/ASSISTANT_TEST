...
async def generate_call_summary(transcript: str) -> str:
    ...
    2. Договоренности и следующий шаг.
    3. Ключевая информация (имя, контакты, специфика).

    Транскрипт:
    {transcript}
    """
    return await ask_yandex_gpt(prompt, session_id="summary_generator")
...
class SIPWorker:
    ...
    async def vad_and_dialog_loop(self):
        ...
        while self.is_running and self.active_rtp_proto == proto:
            ...
            if proto.is_processing or len(proto.pcm_buffer) < 9600:
                continue

            recent_samples = bytes(proto.pcm_buffer[-3200:])
            rms = audioop.rms(recent_samples, 2)

            if rms < SILENCE_THRESHOLD:
                if silence_start_time is None:
                    silence_start_time = time.time()
                elif time.time() - silence_start_time >= 0.5:
